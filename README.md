...
- **Реализация списка:** `UITableView` с переиспользуемыми ячейками `TOTPCell`. Выбран `UITableView`, так как это стандартный UIKit-компонент для отображения однородных списков (у нас именно такой), он обеспечивает переиспользование ячеек для оптимизации памяти, встроенный скролл и pull-to-refresh без дополнительных зависимостей, то есть без избытка покрывает весь необходимый нам функционал. Работа со списком вынесена в `DashboardListManager` (dataSource + delegate), ячейки конфигурируются через `TOTPCellModel` (содержит только строки и флаги для рендера).
- **Состояния экрана:** loading (spinner при первой загрузке), content (таблица с данными), empty (лейбл «Нет TOTP-кодов»), error (текст ошибки + кнопка «Повторить» - например, когда с сетью проблемы и в кэше пусто, то есть совсем никаких данных нет).
- **Навигация:** тап по ячейке - `CodeDetailStubViewController` (заглушка) через `DashboardRouterImpl`.
- **Поиск:** `UISearchController` в navigation bar, фильтрация через `presenter.didSearch(query)`.
- **Как проверить:** Войти с дефолт почтой `user@test.com` и паролем `12345` - увидеть список TOTP-кодов с таймерами, потянуть вниз для обновления, нажать на ячейку для перехода в детали, ввести текст в search bar для фильтрации.
